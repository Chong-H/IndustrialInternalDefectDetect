...
# DATASET
## 金属厚度预测数据集PEC Pusle eddy current
### 注：详细介绍以及数据格式，数据集含义见相关链接
https://www.kaggle.com/datasets/rusuanjun/pec-dataset

通过自主研制的脉冲涡流（PEC）检测设备采集，试验对象包含铝件与 S355 低碳钢件，数据均为一维时序格式。

试件总厚度由叠放板材数量决定：铝板厚度范围 20～60 毫米，梯度为 5 毫米；钢板厚度范围 5～20 毫米，梯度同样为 5 毫米。
此外，该脉冲涡流数据集同时覆盖提离效应与边缘效应两类干扰工况。
数据集详细参数汇总于表 4，针对每一组厚度、提离距离、试件位置组合，均重复采集 50 组样本数据。

多种检测工况会显著影响脉冲涡流检测信号，主要包括提离距离、边缘效应、绝缘层及防护外护套。
因此即便金属板材厚度固定，采集到的信号波形仍会出现明显差异，下文将给出波形对比。

## 超声流量计（Ultrasonic Flowmeters）运行工况与健康状态
### 注：详细介绍以及数据格式，数据集含义见相关链接
https://www.kaggle.com/datasets/amirberenji/ultrasonic-flowmeter-diagnostics-data-set

该数据集最初于 2018 年 1 月 13 日上传至加州大学欧文分校机器学习数据集库（UCI Machine Learning Repository）
内含 4 个数据文件，分别对应超声波流量计的各类运行工况及对应的设备健康状态，可用于故障诊断研究。

## RWDD 焊缝 X 射线照片中的缺陷检测——焊接缺陷检测
### 注：详细介绍以及数据格式，数据集含义见相关链接
链接：https://www.kaggle.com/datasets/viacheslavasadchiy/radiographs-welding-defect-detection?resource=download
...
# 结果
## 注意：针对以下多分类结果，跑出来的评估指标中的查全率，查准率是针对测试集中的各个类别的样本集的各自指标加权平均结果！！！！！！！！！！！！！！
P=Precision 查准率
R=Recall 查全率
F1=P*R/(P+R) 
P=Σwi*Pi  ;wi=ni/N  ;ni为第i类样本数，N为测试集总样本数，Pi为第i类样本的查准率
R=Σwi*Ri  ;wi=ni/N  ;ni为第i类样本数，N为测试集总样本数，Ri为第i类样本的查全率
## 针对PECs两个数据集厚度预测 EPOCH=40
